...
const postCSSPlugin = ({
  ...
  name: "postcss2",
  setup(build) {
    const tmpDirPath = import_tmp.default.dirSync().name, modulesMap = [], cache = new Map();
    ...
    build.onResolve({filter: /.\.(css|sass|scss|less|styl)$/}, async (args) => {
      if (args.namespace !== "file" && args.namespace !== "")
        return;
      let sourceFullPath = import_resolve.default.sync(args.path, {
      ...
      if (cacheVal.outputPath === "") {
        ...
        if (sourceExt === ".less") {
          const ret = await import_less.default.render(new import_util.TextDecoder().decode(fileContent), {
            ...lessOptions,
            filename: sourceFullPath,
...
async function queryCache(sourceFullPath, cache) {
  const fileStat = await (0, import_fs_extra.stat)(sourceFullPath);
  const newCacheVal = {
  ...
  for (const depFile of cacheVal.depFiles) {
    ...
    const depFileStat = await (0, import_fs_extra.stat)(depFile);
...
async function updateDepFilesCache(depFiles, cache) {
  for (const depFile of depFiles) {
    const fileStat = await (0, import_fs_extra.stat)(depFile);
    cache.set(depFile, {
      lastMtimeMs: fileStat.mtimeMs,
      depFiles: [],
      outputCss: "",
